...
def setup_receipts_routes(db, jwt_secret, jwt_algorithm):
    ...
    async def generate_transaction_receipt(
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=recu_sbpay_{transaction_id[:8]}.pdf"
            }
        )
...
def generate_pdf_receipt(transaction: dict, user: dict) -> BytesIO:
    ...
    c.drawString(25*mm, height - 25*mm, "SB Pay")
    ...
    c.drawCentredString(width/2, 15*mm, "Ce document est un reçu officiel généré automatiquement par SB Pay.")
    c.drawCentredString(width/2, 10*mm, f"Généré le {datetime.now().strftime('%d/%m/%Y à %H:%M')} • www.sbpay.com")
